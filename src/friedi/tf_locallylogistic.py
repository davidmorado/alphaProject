...
bandwidth = tf.constant(0.01, dtype="float32")
w = tf.exp(-tf.divide(tf.square(x_ - X), tf.square(tf.multiply(2.0, bandwidth))))

# making the prediction
y_ = tf.reduce_sum(tf.divide(1, 1 + tf.exp(-tf.add(tf.multiply(X, W), b))))

# Mean Squared Error Cost Function 
#cost = tf.reduce_sum(tf.multiply(tf.pow(y_ - Y, 2), w))
#cost = tf.reduce_mean(tf.multiply(-tf.reduce_sum(tf.multiply(Y, tf.log(y_)) + tf.multiply((1-Y), tf.log(1-y_))), w))
...
# Hypothesis function
y_pred = tf.divide(1, 1 + tf.exp(-tf.add(tf.multiply(X, W), b)))
