...
def sq_distance(A, B):
  ...
  return row_norms_A - 2 * tf.matmul(A, tf.transpose(a=B)) + row_norms_B
...
X_test = tf.compat.v1.placeholder(tf.float32, shape = (None,  n_input))
...
cost =tf.reduce_sum(input_tensor=tf.multiply(res,K))
